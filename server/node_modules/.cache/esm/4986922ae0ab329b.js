let mongoose,bcrypt;_e0a‍.x([["default",()=>_e0a‍.o]]);_e0a‍.w("mongoose",[["default",["mongoose"],function(v){mongoose=v}]]);_e0a‍.w("bcrypt",[["default",["bcrypt"],function(v){bcrypt=v}]]);
...
userSchema.pre('save', function (next) {
  ...
  if (user.isModified('password')) {
    return bcrypt.hash(user.password, 12, function (err, hash) {
      if (err) {
        _e0a‍.g.console.log('BCRYPT HASH ERR ', err)
        return next(err)
      }
      user.password = hash
      return next()
...
_e0a‍.d(mongoose.model('User', userSchema));
