let mongoose,bcrypt;_601‍.x([["default",()=>_601‍.o]]);_601‍.w("mongoose",[["default",["mongoose"],function(v){mongoose=v}]]);_601‍.w("bcrypt",[["default",["bcrypt"],function(v){bcrypt=v}]]);
...
userSchema.pre('save', function (next) {
  ...
  if (user.isModified('password')) {
    return bcrypt.hash(user.password, 12, function (err, hash) {
      if (err) {
        _601‍.g.console.log('BCRYPT HASH ERR ', err)
        return next(err)
      }
      user.password = hash
      return next()
...
userSchema.methods.comparePassword = function (password, next) {
  bcrypt.compare(password, this.password, function (err, match) {
    if (err) {
      _601‍.g.console.log('COMPARE PASSWORD ERR', err)
      return next(err, false)
    }
    // if no err, we get null
    _601‍.g.console.log('MATCH PASSWORD', match)
    return next(null, match) // true
  })
}

_601‍.d(mongoose.model('User', userSchema));
